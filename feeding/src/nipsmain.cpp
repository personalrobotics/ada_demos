    ...
    bool adaReal)
{
  ...
  // // auto result = feedingDemo.mAdaMover->planToEndEffectorOffset(direction,
  // 0.180372, false);
  // auto result = feedingDemo.mAdaMover->planToEndEffectorOffset(direction,
  // 0.08, false);
  ...
  while (true)
  {
    ...
    std::cout << "\033[0;32m5) Watermelon\033[0m" << std::endl;
    ...
    */
    ...
    if (!skipSkewering)
    {
      ...
      while (!foodPickedUp)
      {
        ...
        if (angledSkewering)
        {
          feedingDemo.moveAboveFood(foodTransform, 1, viewer, true);
        }
        else
        {
        ...
        if (adaReal)
        {
          if (!feedingDemo.moveIntoFood(&perception, viewer))
          {
            ...
            std::cout << "\033[1;32mOoops! I lost the food! Let me try "
                         "again...\033[0;32m"
                      << std::endl;
            continue;
          }
    ...
    while (!feedingDemo.moveInFrontOfPerson())
    {
      if (!waitForUser(
              "\033[1;32mFailed to move. Try again? ('n' to quit)\033[0;32m"))
      {
