  ...
  , mFTThresholdHelper(ftThresholdHelper)
{
  ...
  zForceAvgDiff = 0.0;
  visionResult = -1;
  numDataPts = getRosParam<int>("/acquisitionDetector/numDataPts", *mNodeHandle);
}
...
bool AcquisitionDetector::collectForqueDataBeforeAction()
{
  std::this_thread::sleep_for(std::chrono::milliseconds(
    getRosParam<int>("/acquisitionDetector/waitMillisecsBeforeCollection", 
                     *mNodeHandle)));
  ...
  if (mFTThresholdHelper)
  {
    ...
    if (canCollect)
    {
      ...
      while (!mFTThresholdHelper->isDataCollectionFinished(beforeForceAvg,
            beforeTorqueAvg)) {
...
bool AcquisitionDetector::collectForqueDataAfterAction()
{
  ...
  if (mFTThresholdHelper && !ftTimeout)
  {
    ...
    if (canCollect)
    {
      ...
      while (!mFTThresholdHelper->isDataCollectionFinished(afterForceAvg,
            afterTorqueAvg)) { }
...
int AcquisitionDetector::autoDetectAcquisition()
{
    ...
    double weight = getRosParam<double>("/acquisitionDetector/weight", *mNodeHandle);
    double forceThreshold = weight * 0.0098;  // weight is set on top of file

    if (visionResult != -1)  // vision is up
    {
      double tpVision = getRosParam<double>("/acquisitionDetector/tpVision", *mNodeHandle);
      double tnVision = getRosParam<double>("/acquisitionDetector/tnVision", *mNodeHandle);
    ...
    if (mu != 0.0)  // haptic is up
    {
      ...
      double sigma = getRosParam<double>("/acquisitionDetector/sigma", *mNodeHandle);
    ...
    if (p >= getRosParam<double>("/acquisitionDetector/upperThreshold", *mNodeHandle))
    {
      return 1;
    }
    else if (p <= getRosParam<double>("/acquisitionDetector/lowerThreshold", *mNodeHandle))
    {
      return 0;
    }
    else
