                ...
                bool adaReal) 
{
  ...
  while(waitForUser("next step?")) {
    ...
    std::cout << std::endl << "\033[1;32mRunning bite transfer study for " << foodName << ".\033[0m" << std::endl;
    ...
    if (foodName == "pickupfork") {
      ...
      if (!skipSkewering) {
        ...
        // 1 strawberry-style
        // 2 banana-style
        int pickupAngleMode = getRosParam<int>("/study/pickupAngleMode", nodeHandle);
        bool angledSkewering;
          ...
          if (pickupAngleMode == 1) {
              feedingDemo.moveAboveFood(foodTransform, pickupAngleMode, viewer, true);
          } else {
            feedingDemo.moveAboveFood(foodTransform, pickupAngleMode, viewer, true);
          }
