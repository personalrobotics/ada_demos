    ...
    ros::NodeHandle nodeHandle)
{
  ...
  ROS_INFO_STREAM("==========  POST HOC EXPERIMENT ==========");
  ...
  while (true)
  {
    ...
    // Get Features from Parameter Server
    std::vector<double> features;
    nodeHandle.getParam("/acquisitionData/features", features);
    ...
    if (ros::service::call("GetAction", getAction)) {
        prob = getAction.response.p_t;
        action = getAction.response.a_t;
    } else {
    ...
    // TODO: Update Bandit
    std::vector<double> posthoc = feedingDemo.getFTThresholdHelper()->getData();
    int loss = success ? 0 : 1;

    posthoc_learn::PublishLoss publishLoss;
    publishLoss.request.visual.insert(std::end(publishLoss.request.visual), std::begin(features), std::end(features));
    publishLoss.request.haptic.insert(std::end(publishLoss.request.haptic), std::begin(posthoc), std::end(posthoc));
    publishLoss.request.p_t.insert(std::end(publishLoss.request.p_t), std::begin(prob), std::end(prob));
    publishLoss.request.a_t = action;
    publishLoss.request.loss = loss;

    if (ros::service::call("PublishLoss", publishLoss))
    {
      ROS_INFO("Success! Error Message: %d", (int)publishLoss.response.success);
    }
    else
    {
